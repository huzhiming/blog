...
```tsx
https://kentcdodds.com/blog/application-state-management-with-react // 应用程序状态管理
https://kentcdodds.com/blog/how-to-use-react-context-effectively // 如何更好的使用context
...
在 React 18 中，我们获得了多种形式的新 API。

`startTransition`— 其目的是将状态更新视为**转换**，从而使处理它们变得不紧急。

`useDeferredValue` — 这个钩子会给你一个传递值的延迟元素，它将跟随原始元素，直到提供超时。

`SuspenseList`— 此组件旨在处理例如获取的数据可能以不可预测的顺序到达的情况。多亏了`<SuspenseList>`，可以对组件进行编排，并且可以控制 UI。



```typescript
// Q: createHostRootFiber 里面的 mode HostRoot ConcurrentMode 代表什么意思
var LegacyRoot = 0;
var BlockingRoot = 1; //阻塞根
var ConcurrentRoot = 2; // 并发根

// 以下函数中创建了几个重要的变量
root = container._reactRootContainer // 
fiberRoot = root._internalRoot; // 内部根
function createFiberRoot(containerInfo, tag, hydrate, hydrationCallbacks) {
  var root = new FiberRootNode(containerInfo, tag, hydrate);
  // stateNode is any.


  var uninitializedFiber = createHostRootFiber(tag); // 本质就是new FiberNode() 得到的实例
...
// root._internalRoot; // 就是FiberRootNode生产出来的
// var root = new FiberRootNode(containerInfo, 0, false);
var root = new FiberRootNode(containerInfo, tag, hydrate);
function FiberRootNode(containerInfo, tag, hydrate) {
  ...
  function FiberNode(tag, pendingProps, key, mode) {
    ...
    this.updateQueue = null; // 在initializeUpdateQueue函数
